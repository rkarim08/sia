...
export async function pullChanges(db: SiaDb, _bridgeDb: SiaDb, config: SyncConfig): Promise<PullResult> {
        if (!config.enabled) {
        ...
        const maxHlcRow = await db.execute("SELECT MAX(hlc_modified) as max_hlc FROM entities WHERE hlc_modified IS NOT NULL");
