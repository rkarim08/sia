...
const pythonPatterns: LanguagePatterns = {
	...
	imports: [/from\s+\S+\s+import\s+(\w+)/gm, /^import\s+(\w+)/gm],
...
const phpPatterns: LanguagePatterns = {
	...
	classes: [/class\s+(\w+)/gm, /interface\s+(\w+)/gm, /trait\s+(\w+)/gm],
			...
			while (m !== null) {
				...
				if (category === "call") {
					...
					facts.push({
						...
						confidence: 0.92,
						extraction_method: "regex-ast",
						proposed_relationships: [{ target_name: name, type: "calls", weight: 0.7 }],
					});
					m = regex.exec(content);
					continue;
				}
