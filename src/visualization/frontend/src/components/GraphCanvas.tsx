...
import { BG_PRIMARY, COMMUNITY_COLORS, DEFAULT_VISIBLE_TYPES, NODE_COLORS } from "../lib/constants";
import {
	computeRadialLayout,
	computeTreeLayout,
	filterGraphByDepth,
...
import type { LayoutMode, SigmaEdgeAttributes, SigmaNodeAttributes, ViewBookmark } from "../types";
...
const GraphCanvas = forwardRef<GraphCanvasHandle, Props>(
	(
		...
		useEffect(() => {
			if (!graph) return;
			const visibleTypes = DEFAULT_VISIBLE_TYPES.filter((t) => !hiddenTypes.has(t));
			if (activeFolder) {
		...
		const blastDistances = useMemo(() => {
			if (!blastRadiusMode || !selectedNodeId || !graph || !graph.hasNode(selectedNodeId))
				return null;
		...
		const {
			...
			setCameraState,
			sigmaRef,
		} = useSigma(containerRef as React.RefObject<HTMLDivElement>, graph, {
		...
		useImperativeHandle(ref, () => ({ focusNode, exportPNG, getCameraState, setCameraState }), [
			focusNode,
			exportPNG,
			getCameraState,
			setCameraState,
			...
			const handleKeyDown = (e: KeyboardEvent) => {
				...
				if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) {
					...
					const neighbors = graph
						...
						.filter((n) => !graph.getNodeAttributes(n).hidden);
		...
		const handleLocalGraphClick = useCallback(
			(e: React.MouseEvent<HTMLCanvasElement>) => {
				if (!graph || !selectedNodeId || !graph.hasNode(selectedNodeId) || !localGraphRef.current)
					return;
		...
		const visibleCount = graph
			? graph.nodes().filter((n) => !graph.getNodeAttributes(n).hidden).length
			: 0;
		...
		return (
			<div style={{ position: "relative", width: "100%", height: "100%" }}>
				...
				<div
					style={{
						position: "absolute",
						bottom: Math.round(36 * lgScale),
						left: 0,
						right: 0,
						height: Math.round(24 * lgScale),
						display: "flex",
						alignItems: "center",
						...
						fontSize: isMobile ? 9 : Math.round(10 * lgScale),
						fontFamily: "'GeistMono', 'Geist Mono', monospace",
