...
import type { SigmaEdgeAttributes, SigmaNodeAttributes } from "../types";
import type { GraphCombo, GraphNode, GraphResponse } from "./api";
import { EDGE_INFO, NODE_COLORS, NODE_SIZES, type SiaNodeType } from "./constants";
 ...
 */
const getNodeMass = (nodeType: SiaNodeType, isFolder: boolean, nodeCount: number): number => {
...
export const graphResponseToGraphology = (
	...
	combos.forEach((combo, index) => {
		...
		const radius = structuralSpread * Math.sqrt((index + 1) / Math.max(combos.length, 1));
	...
	const addNode = (node: GraphNode) => {
		...
		const cluster = node.parentId ? getCluster(node.parentId, comboMap) : "";
	...
	const edgeBaseSize = totalElements > 20000 ? 0.4 : totalElements > 5000 ? 0.6 : 1.0;
...
export const filterGraphByFolder = (
	...
	graph.forEachNode((nodeId, attrs) => {
		...
		const inFolder =
			folderIds.has(nodeId) || (attrs.parentId ? folderIds.has(attrs.parentId) : false);
 ...
 */
export const computeTreeLayout = (graph: Graph<SigmaNodeAttributes, SigmaEdgeAttributes>): void => {
