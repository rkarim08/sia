...
export function useSigma(
	...
	useEffect(() => {
		...
		const renderer = new Sigma(graph, containerRef.current, {
			...
			defaultDrawNodeHover: (context, data, settings) => {
				...
				const nodeAttrs = nodeId && graph.hasNode(nodeId) ? graph.getNodeAttributes(nodeId) : null;
				...
				const maxContentWidth = Math.max(labelWidth, badgeWidth + 14 + infoWidth + trustDotsWidth);
				...
				if (blastMode && blastDistances && selected) {
					...
					if (distance !== undefined) {
						...
						const sizeMultiplier =
							distance === 0 ? 2.0 : distance === 1 ? 1.4 : distance === 2 ? 1.1 : 0.8;
		...
		renderer.on("rightClickNode", ({ node, event }) => {
			...
			const clientX =
				"clientX" in original
					? original.clientX
					: ((original as TouchEvent).touches?.[0]?.clientX ?? 0);
			const clientY =
				"clientY" in original
					? original.clientY
					: ((original as TouchEvent).touches?.[0]?.clientY ?? 0);
	...
	const focusNode = useCallback(
		(nodeId: string) => {
			...
			sigma
				.getCamera()
				.animate({ x: nodePosition.x, y: nodePosition.y, ratio: 0.15 }, { duration: 400 });
		},
		[graph],
	);
