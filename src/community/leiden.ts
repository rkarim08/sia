...
function dedupeEdges(edges: EdgeRow[], validNodes: Set<string>): Array<{ a: string; b: string; w: number }> {
...
export async function detectCommunities(db: SiaDb, opts: LeidenOpts = {}): Promise<CommunityResult> {
        ...
        for (let level = 0; level < resolutions.length; level++) {
                ...
                for (const [key, members] of communityMembers) {
                        const id = randomUUID();
