...
export async function getStats(
	...
	const { rows: entityTypeRows } = await graphDb.execute(
		"SELECT type, COUNT(*) AS cnt FROM entities WHERE t_valid_until IS NULL AND archived_at IS NULL GROUP BY type",
	);
	...
	const { rows: archivedRows } = await graphDb.execute(
		"SELECT COUNT(*) AS cnt FROM entities WHERE archived_at IS NOT NULL",
	);
	const archivedCount = (archivedRows[0]?.cnt as number) ?? 0;

	// --- Invalidated count ---
	const { rows: invalidatedRows } = await graphDb.execute(
		"SELECT COUNT(*) AS cnt FROM entities WHERE t_valid_until IS NOT NULL",
	);
	...
	const { rows: edgeTypeRows } = await graphDb.execute(
		"SELECT type, COUNT(*) AS cnt FROM edges WHERE t_valid_until IS NULL GROUP BY type",
	);
