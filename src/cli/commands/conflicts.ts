...
export async function resolveConflict(db: SiaDb, groupId: string, keepEntityId: string): Promise<void> {
        ...
        await db.execute("UPDATE entities SET conflict_group_id = NULL WHERE conflict_group_id = ?", [groupId]);
}
