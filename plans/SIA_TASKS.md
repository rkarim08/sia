...
- `local_dedup_log` table (for nightly consolidation sweep — separate from sync_dedup_log)
...
`src/security/staging-promoter.ts`. For `pending` staged facts: run all three checks sequentially. Pass → promote via standard consolidation pipeline, update status to `passed`. Fail → `quarantined` with reason. Run as part of nightly background job. Log all outcomes to `audit_log`.
...
**Goal:** Nightly decay, archival, consolidation sweeps, episodic-to-semantic promotion, sia_flag system, sharing rules enforcement.  
**Estimated effort:** 14–18 hours

---

**Task 11.1 — Importance decay nightly job** [BLOCKING] — *3–4h*

`src/decay/decay.ts`. Iterate all non-archived, non-invalidated entities (`WHERE archived_at IS NULL AND t_valid_until IS NULL`). Apply formula from ARCHI §8.1. Batch updates (500 at a time). Scheduled by `src/decay/scheduler.ts`. When sync is enabled: aggregate `access_count` across all peers' `SYNC_RECV` entries for this entity to give team-visible entities credit for accesses from all team members.
...
**Task 11.2 — Archival and nightly consolidation sweep** — *3–4h*
...
Acceptance criteria: Soft-archived entities excluded from retrieval, bi-temporally invalidated entities NOT soft-archived (they have their own mechanism), nightly sweep writes to `local_dedup_log` (not `sync_dedup_log`), cached pairs in `local_dedup_log` not re-analyzed. See also Task 11.7 for bridge edge orphan cleanup, which runs in the same nightly scheduler slot.
