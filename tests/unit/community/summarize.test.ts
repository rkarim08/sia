...
describe("summarizeCommunities", () => {
	it("creates summaries and refreshes when membership changes", async () => {
		...
		await db.execute("UPDATE communities SET member_count = member_count + 1 WHERE id = ?", [
			community.id,
		]);
		...
		await db.execute("UPDATE communities SET member_count = member_count + 2 WHERE id = ?", [
			community.id,
		]);
