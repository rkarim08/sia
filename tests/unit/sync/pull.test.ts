...
describe("pullChanges", () => {
	...
	it("writes SYNC_RECV audit entries after pull", async () => {
		...
		await db.execute(ENTITY_INSERT, [
			"e1", "Concept", "TestEntity", "some content about testing", "test summary", null,
			'["test"]', "[]", 3, 0.7, 0.7, 0.5, 0.5, 0, 0, now, now, now, null, null, null,
			"team", "dev", null, null, 200, 100,
	...
	it("persists HLC after pull when repoHash is provided", async () => {
		...
		await db.execute(ENTITY_INSERT, [
			"e1", "Concept", "HlcTest", "content", "summary", null,
			"[]", "[]", 3, 0.7, 0.7, 0.5, 0.5, 0, 0, now, now, now, null, null, null,
			"team", "dev", null, null, 300, 100,
			null, null, null, null, null, null,
		]);
