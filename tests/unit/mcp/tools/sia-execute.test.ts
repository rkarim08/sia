...
describe("handleSiaExecute", () => {
  ...
  it("executes bash code and returns stdout", async () => {
    const deps = setup();
    const result = await handleSiaExecute(db, {
      code: 'echo "hello"', language: "bash",
    }, deps.embedder, deps.throttle, deps.sessionId);
  ...
  it("returns error when throttled to blocked", async () => {
    ...
    const result = await handleSiaExecute(db, {
      code: 'echo "blocked"', language: "bash",
    }, deps.embedder, deps.throttle, deps.sessionId);
  ...
  it("applies context mode for large output with intent", async () => {
    ...
    const result = await handleSiaExecute(db, {
      code, language: "bash", intent: "error lines",
    }, deps.embedder, deps.throttle, deps.sessionId, {
      sandboxTimeoutMs: 10000, sandboxOutputMaxBytes: 1_048_576,
      contextModeThreshold: 1024, contextModeTopK: 3,
    });
